...
GOT_OFFSET_HEX=$(dd if=/proc/$PID/mem bs=1 skip=$((PLT_JMP_ADDR + 2)) count=4 2>/dev/null | xxd -p)
if [ -z "$GOT_OFFSET_HEX" ]; then
  echo "Failed to read GOT offset from memory"
  if [ ! -f ./find_got ]; then
  ...
  GOT_OFFSET_DEC=$(python3 -c "import struct; print(struct.unpack('<i', bytes.fromhex('$GOT_OFFSET_HEX'))[0])")
  GOT_ADDR=$((PLT_JMP_ADDR + 6 + GOT_OFFSET_DEC))
fi
